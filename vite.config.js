...
export default defineConfig(({ mode }) => {
  ...
  if (missingVars.length > 0 && mode === 'production') {
    console.error(
      `[Vite Build] ERROR: Missing required environment variables: ${missingVars.join(', ')}`
    );
  }
  ...
  console.log(
    '  VITE_SUPABASE_URL:',
    supabaseUrl ? `${supabaseUrl.substring(0, 30)}... (length: ${supabaseUrl.length})` : 'MISSING'
  );
  console.log(
  ...
  return {
    ...
    build: {
      // es2019: esbuild transpiles ?. and ?? away, making the bundle parseable by
      // Puppeteer 1.x's bundled Chrome 77 (used by vite-plugin-prerender at build time).
      // Modern browsers support all ES2019 features — no runtime impact.
      target: 'es2019',
      cssTarget: 'chrome100',
      sourcemap: true,
