  ...
  return (
    <div className="flex gap-3 mt-4">
      <button
        ...
        className="flex items-center gap-1.5 bg-background hover:bg-muted border border-border text-foreground font-medium py-2 px-3 rounded-md transition-colors text-xs"
      >
        {copied ? (
      ...
      <div className="relative" ref={exportMenuRef}>
        ...
        {generating && (
          <div className="absolute -top-16 left-0 right-0 bg-card border border-border rounded-lg shadow-lg p-3 z-50">
        ...
        <button
          ...
          disabled={generating}
          className={`flex items-center gap-1.5 font-medium py-2 px-3 rounded-md transition-colors text-xs disabled:cursor-not-allowed ${
            exportSuccess
              ? 'bg-gain text-white hover:opacity-90'
              : generating
        ...
        {/* Dropdown Menu */}
        {exportMenuOpen && (
          <div className="absolute left-0 top-full mt-2 w-52 bg-card rounded-lg shadow-lg border border-border py-1 z-50">
            {/* PDF Export */}
            <button
              onClick={handleDownloadPDF}
              className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-foreground hover:bg-muted transition-colors"
            >
              <div className="w-8 h-8 rounded-md bg-loss-bg flex items-center justify-center flex-shrink-0">
              ...
              <div className="text-left">
                ...
                <div className="text-xs text-muted-foreground">Professional report with analysis</div>
              </div>
            </button>
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-foreground hover:bg-muted transition-colors"
            >
              <div className="w-8 h-8 rounded-md bg-gain-bg flex items-center justify-center flex-shrink-0">
                <svg className="w-4 h-4 text-gain" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <div className="text-left">
                <div className="font-medium">CSV (Holdings)</div>
                <div className="text-xs text-muted-foreground">Current portfolio data</div>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left">
                ...
                <div className="text-xs text-muted-foreground">Rebalancing recommendations</div>
              </div>
            </button>
          </div>
        )}
