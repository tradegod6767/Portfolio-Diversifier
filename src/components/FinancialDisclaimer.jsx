...
export default function FinancialDisclaimer({ variant = 'inline', className = '' }) {
  ...
  if (variant === 'banner') {
    return (
      <div className={`bg-amber-50 border-l-4 border-amber-500 rounded-lg p-4 md:p-6 shadow-sm ${className}`}>
  ...
  if (variant === 'footer') {
    return (
      <div className={`text-xs text-slate-400 leading-relaxed ${className}`}>
        <p className="mb-2">
    ...
    <div className={`bg-slate-50 border border-slate-200 rounded-lg p-4 ${className}`}>
      <div className="flex items-start gap-2">
        ...
        <p className="text-xs text-slate-600 leading-relaxed">
          <strong className="font-semibold text-slate-700">Disclaimer:</strong> {disclaimerText}
        </p>
      </div>
    </div>
  );
